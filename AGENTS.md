...
- Archiving and deleting a task are two mechanisms and must stay two: `archivedAt` (reversible, no expiry, `src/lib/utils/issueArchive.mjs`) and the `deletedIssues` tombstone (24-hour window, `src/lib/utils/issueTrash.mjs`). A new reader of issues excludes archived tasks unless it is the archive itself. In user-facing copy the second one is «Нещодавно видалене», never «Кошик».
- A sprint's `projectIds` is its scope; empty means the whole organization. Several sprints may be active at once, but not two over the same project — `src/lib/utils/sprintScope.mjs` is the only place that decides either question.

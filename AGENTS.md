...
- Archiving and deleting a task are two mechanisms and must stay two: `archivedAt` (reversible, no expiry, `src/lib/utils/issueArchive.mjs`) and the `deletedIssues` tombstone (24-hour window, `src/lib/utils/issueTrash.mjs`). In user-facing copy the second one is «Нещодавно видалене», never «Кошик».

...
Calendar and deadline notifications are generated by the protected `/api/cron/notifications` job, so they do not depend on anyone having QuickTeam open. The schedule lives in `.github/workflows/scheduled-notifications.yml` rather than `vercel.json`, because Vercel's Hobby plan allows a cron job only once per day — too coarse for a reminder that must arrive minutes before an event. Set `CRON_SECRET` in **both** places: as a production environment variable, which the route validates, and as a repository secret, which the workflow sends as a bearer token. A mismatch answers 401 and the workflow run fails.

GitHub does not honour `*/5 * * * *` either: measured on this repository it starts a run every 60–210 minutes. Two mechanisms cover that. The sweep records its own watermark in `system/notificationSweep` and asks for everything since its previous pass — clamped to twelve hours — so a late run delivers what it missed instead of discarding it, and a failed run does not advance the watermark. And one workflow run sweeps every two minutes for fifty minutes rather than firing once, with `cancel-in-progress` so the next scheduled run replaces it. Reminder text is computed from the distance to the event at delivery, so a late reminder says it is late. Moving the trigger to a host that honours a schedule remains the durable fix.

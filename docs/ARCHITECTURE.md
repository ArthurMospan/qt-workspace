...
випливає все: такий запис не дзвенить, не спливає карткою і гаситься
(`read: true`) тієї ж миті, як прийшов, ще до того, як список намальовано, тому
лічильник не блимає «+1». Коли розмова перед читачем змінюється або вкладка
повертається, міст кличе `settleVisible()`, і записи про те, що тепер на екрані,
гаснуть так само. Правило одне для всіх типів, які продукує розмова; виняток —
`emergency`, `alert` і `test`: екстрений виклик має перебивати будь-що і не
гаситься мовчки.

...
**Чого ще не зроблено.** Головна досі рахує прогрес із задач. Спершу лічильники
й звірка (`npm run backfill:project-issue-counts`, dry run на проді, див.
[MIGRATIONS.md](MIGRATIONS.md)), і лише коли аудит стабільно каже «розбіжностей:
0» — екран перестає читати задачі. Прибрати розрахунок раніше означало б не мати
з чим порівняти.

...
- двері — пароль зі змінної `ERROR_REPORTS_PASSWORD`. Він перевіряється на
  сервері, порівнюється як дайджест сталої довжини, обмежений десятьма спробами
  на адресу за пʼять хвилин і ніколи не потрапляє в браузер. Поки змінної немає,
  сторінка закрита: сторінка, що відчиняється через відсутність змінної, не є
  зачиненою сторінкою;

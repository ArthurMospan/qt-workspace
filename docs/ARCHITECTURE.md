...
Сповіщення завжди записується — сервер не бачить чужих екранів і пише кожен
запис непрочитаним. Що з цим записом робити далі, вирішує один читач, який
екран бачить: `useNotifications`. Панелі, що показують розмову, публікують її у
стор — `visibleConversations` як стек, `visibleConversation` як його верхівка
(`{ kind: 'issue' | 'dm' | 'channel', id }`). Стек, а не одна комірка, бо задача
відкривається вікном поверх чату, і після її закриття канал під нею й далі на
екрані. `WorkspaceNotificationBridge` віддає хуку одну відповідь
`readerIsWatching` — вкладка попереду і `isConversationOnScreen` — і з неї
випливає все. Сповіщення — це про те, що сталося без тебе, тому запис, що
прийшов у розмову на екрані, сповіщенням не є: він не дзвенить, не спливає
карткою і **видаляється** (`witnessedRecordIds`) тієї ж миті, як прийшов, ще до
того, як список намальовано, — сама розмова і є записом. Запис, що вже чекав
непрочитаним, коли читач відкрив розмову, своє зробив — привів сюди — і
**гаситься** (`settleRecordsOnScreen`, `read: true`): лишається в «Усі» як
прочитаний, це журнал того, про що дзвоник казав. Коли розмова перед читачем
змінюється або вкладка повертається, міст кличе `settleVisible()`, і записи, що
чекали на те, що тепер на екрані, гаснуть так само. Правило одне для всіх типів,
які продукує розмова; виняток — `emergency`, `alert` і `test`: екстрений виклик
має перебивати будь-що і не зникає мовчки. Вкладка у фоні — це «тебе там немає»:
такий запис лягає в дзвоник і чекає.

Сирена екстреного виклику звучить тричі за шість секунд і лише тоді, коли
виклик іще «зараз»: для запису, молодшого за десять хвилин, і один раз на
браузер — які записи вже звучали, пам'ятає localStorage (`emergencyAlarm.mjs`).
Раніше пам'ять жила в ref і зникала з перезавантаженням, тож непрочитаний
виклик триденної давності зустрічав кожне оновлення сторінки сиреною. Інший
пристрій звучить сам — людина могла не чути першого; червоний запис у дзвонику
лишається, доки його не прочитають, і прочитання, як і раніше, зупиняє повтори.

Самі сторінки записів у дзвонику не торкаються — ні задача, ні чат, ні бічна
панель. Раніше кожна тримала свій список типів, а сайдбар при вході в проєкт
гасив усі його записи разом і читав для цього всю колекцію сповіщень, тож
сповіщення про задачі, яких людина не відкривала, зникали, щойно вона зайшла в
сусідню. Крапка біля проєкту в рейці гасне тоді, коли прочитано записи за ним —
у відкритій задачі або в дзвонику. Яку саме розмову називає запис, відповідає
`notificationConversationId` — з поля `channelId`, а для записів, створених до
появи цього поля, з посилання (`/chat?channel=…`, `/chat?dm=…`).

Дзвоник тримає вікно з п'ятдесяти найновіших записів. Поки вікно неповне, число
на дзвонику — з нього і точне. Повне вікно може не бачити непрочитаного,
старшого за п'ятдесятий запис, тому тоді число бере `notificationUnreadByOrg` —
той самий серверний `count()`, що й екран вибору організації. «Позначити все
прочитаним» і «Очистити прочитані» запитують лише записи з потрібним `read`, за
індексом `userId + organizationId + read`, а не всю колекцію.
...
документі задачі — це поле, за яке платить кожна дошка, кожен список і «Мої
завдання», незалежно від того, чи його хтось малює. `importMetadata` був

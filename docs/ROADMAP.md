...
### Перенесення з YouTrack: два кроки, які лишились

Екран перенесення переписаний навколо стану job: розвідка й вибір живуть на
сервері, натискань до першого запису два, і в жодного стану немає виходу — це
тримає тест у `tests/youtrack-import.test.mjs`. Деталі в
[docs/integrations/YOUTRACK_MIGRATION.md](integrations/YOUTRACK_MIGRATION.md).
Лишилось рівно двоє, і порядок між ними обов'язковий.

**1. Звузити читання фази зв'язків. Робити першим.** `processPendingLink`
(`src/lib/server/youtrackImporter.js`) бере в транзакцію всі `issueLinks`
організації та всі `issues` цільового проєкту — на кожен зв'язок. Після імпорту
663 задач в один проєкт це ≥ 663 читання на зв'язок, тобто понад 130 000 читань
на 200 зв'язків: 2,6 денної стелі Spark за один прохід. Обидва читання потрібні
по-справжньому — org-wide скан шукає старий зв'язок для тієї самої пари, а граф
проєкту живить пошук циклу, — тож звуження треба робити разом із
`tests/issue-relations.test.mjs`, а не «оптимізацією». Найпростіший чесний хід:
підняти обидва читання з транзакції на рівень пакета кроків і перечитувати їх
раз на пакет, а не раз на зв'язок.

**2. Серверний драйвер.** Зараз перенесення веде вкладка: один HTTP-запит на
задачу, ~665 запитів і півгодини з відкритим екраном на 663 задачі. Два кроки,
обидва доступні: спершу пакетний крок — крутити цикл усередині одного виклику,
доки не витрачено ~50 із 60 секунд (це саме по собі ділить кількість запитів на
двадцять і знімає фіксовані читання авторизації та rate-limit з кожної задачі);
потім прохід із наявного циклу GitHub Actions, який і так стукає в застосунок
раз на хвилину майже безперервно (`.github/workflows/scheduled-notifications.yml`,
355 хвилин на запуск). Прохід без користувача мусить отримати явний шлях «веде
планувальник від імені `createdBy`», а не обхід `assertImportControl`, і
`refuseWithoutCapability` лишається на кожен job. **Не вмикати, доки не зроблено
(1):** нічний драйвер поверх нинішньої фази зв'язків тихо з'їдає денний бюджет
читань цілої організації.

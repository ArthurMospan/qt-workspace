...
**Постійне рішення — TTL-політика Firestore на полі `resetAt`** у цій колекції:
вона видаляє прострочене сама, не витрачає квоту записів і не потребує коду.
Робиться в консолі Firebase → Firestore → TTL.

...
test('кожна платна можливість має двері, а не лише екран', async () => {
  const doors = {
    ...
    'data-import': [
      ...
      // Третій, і його бракувало. Розвідка — найдорожчий роут цієї інтеграції
      // (сотні звернень до чужого API з нашого сервера), і питали про тариф
      // лише підключення та імпорт. Поки її запускали кнопкою, це було
      // непомітно; тепер її запускає відкриття екрана.
      'src/app/api/integrations/youtrack/discover/route.js',
    ],

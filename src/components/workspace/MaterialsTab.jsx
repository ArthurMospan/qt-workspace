    ...
    <div className="flex-1 overflow-y-auto bg-canvas">
      <div className="max-w-[860px] mx-auto px-6 py-5">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-[16px] font-bold text-ink">Матеріали проєкту</h2>
            <p className="text-[12px] text-muted mt-[2px]">Внутрішні файли, посилання та документи команди</p>
          </div>
        ...
        {/* Add form */}
        {showAdd && (
          <div className="bg-white border border-line rounded-[16px] p-5 mb-5">
            <p className="text-[13px] font-semibold text-ink mb-3">Новий матеріал</p>
            <div className="flex flex-col gap-3">
              <div>
                <label className="text-[11px] font-semibold text-muted uppercase tracking-wide mb-1 block">URL або посилання</label>
                <Input
                  autoFocus
                  value={addUrl}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-semibold text-muted uppercase tracking-wide mb-1 block">Назва</label>
                  <Input
                    value={addName}
                ...
                <div>
                  <label className="text-[11px] font-semibold text-muted uppercase tracking-wide mb-1 block">Нотатка</label>
                  <Input
                    value={addNote}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16 text-center">
            <FolderOpen size={36} className="text-line mb-3" />
            <p className="text-[14px] font-semibold text-faint mb-1">Матеріалів немає</p>
            ...
            {filtered.map((file, i) => {
              ...
              return (
                <div key={file.id}
                  ...
                  {/* Preview */}
                  <div className="w-[40px] h-[40px] bg-canvas rounded-[8px] shrink-0 overflow-hidden flex items-center justify-center border border-line">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-[2px]">
                      {file.note && <p className="text-[11px] text-muted truncate">{file.note}</p>}
                      <span className="text-[10px] text-faint flex items-center gap-1 shrink-0">
                        <Calendar size={9} />
                        {fmtDate(file.createdAt)}
                      </span>
                      {file.addedByName && (
                        <span className="text-[10px] text-faint shrink-0">{file.addedByName}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    {file.url && (
                      <a href={file.url} target="_blank" rel="noopener"
                        className="p-[6px] text-muted hover:text-ink hover:bg-[#f0f0f0] rounded-[6px] transition-all">
                        <ExternalLink size={13} />
                      </a>
                    )}
                    <button onClick={() => handleDelete(file.id)}
                      className="p-[6px] text-faint hover:text-red-500 hover:bg-red-50 rounded-[6px] transition-all">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </div>

 ...
 * @param {'trigger'|'choice'} props.size Which of the two roles this dot plays.
 * @param {boolean} props.selected `choice` only: this is the colour currently picked.
...
export default function ColorSwatch({
  ...
  return (
    <button
      type="button"
      aria-label={label}
      aria-pressed={size === 'choice' ? selected : undefined}
      onClick={onClick}
      style={{
        background: color,
      ...
      className={`${SIZES[size] ?? SIZES.choice} ${className}`}
      {...props}
    />
  );
}

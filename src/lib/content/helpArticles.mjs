...
export const HELP_ARTICLES = Object.freeze([
  ...
  },
  {
    ...
    keywords: ['проєкт', 'дошка', 'колонки', 'команда', 'архів', 'фільтр', 'посилання', 'закладка'], updatedAt: UPDATED_AT,
    ...
    sections: [
      ...
      { id: 'table', title: 'Таблиця', paragraphs: ['Третій вигляд проєкту показує задачі рядками, а поля — колонками. Заголовок таблиці й колонка з ключем та назвою лишаються на місці під час прокрутки. Клік по клітинці змінює значення прямо в ній тим самим контролом, що й у картці задачі; ключ ліворуч лишається посиланням на саму задачу. В архівованому проєкті клітинки не відкриваються.'], bullets: ['«Колонки» вмикає тип, спринт, мітки, оцінку, чекліст, коментарі, блокування, «Створено» і «Оновлено»; ключ і назву вимкнути не можна.', 'Клік по заголовку колонки: за зростанням → за спаданням → назад до порядку дошки. Задача без значення в цій колонці завжди остання.', '«Групувати» ділить таблицю на смуги за статусом, виконавцем, пріоритетом, типом або спринтом; сортування діє всередині смуги.', 'Задача з кількома виконавцями належить смузі першого з них.', 'Прапорці ліворуч і Shift дають той самий масовий вибір, що на дошці й у списку.'] },
      { id: 'filters', title: 'Фільтри й посилання', paragraphs: ['Обрані фільтри та вибір «дошка / список / таблиця» показані в адресі сторінки. Тому дошку з фільтрами можна додати в закладки, надіслати колезі або відкрити на іншому пристрої — вона відкриється рівно такою, якою ви її бачили. Кнопка «назад» у браузері скасовує останню зміну фільтра.'], bullets: ['Так само працюють «Мої завдання» та «Спринти».', 'Посилання з фільтрами завжди має перевагу: воно показує стан відправника, а не ваш попередній.', 'Адреса без фільтрів відновлює ваш попередній перегляд цього екрана.', 'Фільтр у стані «всі» в адресі не показується.', 'Колонки, сортування й групування таблиці їдуть в адресі так само, тож налаштована таблиця — це теж посилання. Обрані колонки не губляться, коли перемкнутись на дошку й повернутись.'] },
  ...
  },
  {
    ...
    sections: [
      ...
      { id: 'add', title: 'Як задача потрапляє у спринт', paragraphs: ['Задачу можна перетягнути зі списку «Без спринта» у потрібний спринт, обрати спринт у самій задачі або натиснути «+» у шапці спринта — тоді форма створення відкривається з уже підставленим спринтом. Останній спосіб є єдиним на телефоні, де перетягування картки між двома списками одне під одним неможливе.'] },
  ...
  },
  {
    ...
    sections: [
      ...
      { id: 'task-feed', title: 'Чат задачі', paragraphs: ['Чат усередині задачі несе не лише повідомлення: там же стоять її зміни та списаний час. Смуга «Нове в задачі» рахує і повідомлення, і зміни, а повідомлення позначаються прочитаними лише коли смуга справді була на екрані — див. «Профілі й онлайн-статус».'] },
  ...
  },
  {
    ...
    title: 'Профілі й онлайн-статус', category: 'collaboration', summary: 'Команда, профіль учасника, presence, історія задачі та прочитано/непрочитано.',
    keywords: ['профіль', 'команда', 'presence', 'остання активність', 'audit', 'прочитано', 'непрочитано', 'історія задачі'], updatedAt: UPDATED_AT,
    ...
    sections: [
      ...
      {
        id: 'issue-history',
        title: 'Історія задачі в її ж чаті',
        paragraphs: ['Зміни задачі стоять у тій самій смузі, що й повідомлення та списаний час: хто змінив, що саме змінилось і на що. У журнал потрапляють статус, назва, виконавці, пріоритет, тип, дедлайн, оцінка, мітки, спринт і сам факт зміни опису. Статуси й пріоритети підписані назвами вашого проєкту, а не спільними: перейменований статус читається так, як ви його назвали.'],
      },
      {
        id: 'read-state',
        title: 'Прочитано й непрочитано',
        paragraphs: ['Крапка на картці, у рядку списку й у таблиці означає «тут є нове для вас» — і однаково рахує повідомлення та зміни. Наведіть на неї, щоб побачити, про що йдеться. Усередині задачі те саме показує смуга «Нове в задачі»: усе під нею з’явилось після вашого попереднього візиту.'],
        bullets: [
          'Задача стає прочитаною, коли ви її закриваєте, а не коли відкрили: відкрив і відвернувся — і межа лишається на місці.',
          'Власні зміни ніколи не позначаються як нові — ні крапкою, ні смугою.',
          '«Позначити непрочитаним» у меню задачі повертає крапку й межу, щоб повернутись до задачі пізніше. Пункт з’являється лише тоді, коли останню дію зробив хтось інший.',
        ],
      },
    ],
  },

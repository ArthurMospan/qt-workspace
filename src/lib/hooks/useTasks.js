...
export function useTasks(projectId) {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'tasks'), where('organizationId', '==', activeOrgId), where('projectId', '==', projectId));
  ...
  const moveTask = useCallback(async (taskId, newStatus, newOrder) => {
    ...
    const destTasks = tasks.filter(t => t.id !== taskId && t.status === newStatus).sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
